...
const tempContent = process.env.QS_TEMP_CONTENT_DIRS;
const dirs = tempContent.split("|");
const tempContentArchive = process.env.QS_TEMP_CONTENT_ARCHIVE_DIR;
    ...
    .on("add", async function (file, stats) {
        ...
        if (path.extname(file) === ".xlsx") {
            if (!renamed.includes(fileName)) {
                ...
                const worker = new Worker(__dirname + "/worker.js", { workerData: { file, tempContentArchive, timeLimit: timeLimitInSec * 1000 } });
                ...
                worker.on("exit", code => {
                    code === 0 ? logger.info(`${resourceId} | ${fileName} | SUCCESS`) : logger.warn(`${resourceId} | ${fileName} | FAIL`);
                });
            }
        } else {
